...
def contruct_street_networks(Gs, network_data_pth, place_name, placeid):
...
def construct_top_routes_network(Gs, network_data_pth, placeid, place_name):
    ...
    for layer, streets in routes.items():
        ...
        Gs[layer] = G_filtered
        ox_to_csv(Gs[layer], network_data_pth, placeid, "toproutes", postfix= f"{layer}")
    for layer in list(routes.keys()):
        #G_temp = nx.MultiDiGraph(ox.utils_graph.get_digraph(ox.simplify_graph(Gs[parameterid]))) # This doesnt work - cant get rid of multiedges
        ox_to_csv(ox.simplify_graph(Gs[layer]),network_data_pth,placeid ,f"toproutes_{layer}", "simplified")
...
if __name__ == "__main__":
    ...
    placeid = "Bucharest"
    Gs = {}
    construct_top_routes_network(Gs, network_data_pth, placeid, place_name)