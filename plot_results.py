...
def plot_GT_visualizations(map_center, G_carall, res, plots_pth, plotparam, nnids, nodesize_poi, placeid, poi_source, prune_quantile, prune_measure, weight_abstract, nodesize_grown):
...
def plot_population_density_squares(plots_pth, squares, map_center, G_carall, placeid, poi_source, prune_measure):
    ...
    axes.plot()
    nxdraw(G_carall, "carall", map_center)
    plt.savefig(plots_pth/f"TEST.png", bbox_inches="tight", dpi=plotparam["dpi"])
...
if __name__ == "__main__":
    ...
    with open(analysis_results_pth/f"{placeid}_poi_{poi_source}_Bqtoproutes_covers.pickle",'rb') as f:
    ...
    population_cov = Polygon()
    squares_densities = []
    for square in squares:
        population_cov = ops.unary_union([population_cov, squares[square]["square_wgs84"]])
        squares_densities.append(squares[square]['density'])
    squares_densities = sorted(squares_densities)
    min = squares_densities[0]
    q1 = np.percentile(squares_densities, 33)
    q2 = np.percentile(squares_densities, 66)
    max = squares_densities[-1]
    print(min, q1, q2, max)
    pc_population = plot_population_density_squares(plots_pth/"covs", squares, map_center, G_carall, placeid, poi_source, prune_measure)
    
    #plot_GT_covs(plots_pth/"covs", map_center, G_carall, placeid, nnids,nodesize_poi, res, covs_GT, cov_car, prune_measure,squares ,pc_population)
    #plot_MST_covs(plots_pth/"covs", map_center, G_carall, placeid, nnids, nodesize_poi, res, covs_MST, cov_car, prune_measure,squares ,pc_population)
    #plot_SUMP_covs(plots_pth/"covs",map_center, G_carall, placeid, nnids, nodesize_poi, Gs_bikenetwork, covs_SUMP, cov_car, prune_measure, prune_quantiles_SUMP,squares ,pc_population) 